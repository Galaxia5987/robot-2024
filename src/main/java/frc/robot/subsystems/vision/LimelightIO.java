...
public class LimelightIO implements VisionIO {
    ...
    private VisionResult result = new VisionResult(
            new EstimatedRobotPose(new Pose3d(), 0,
                    new ArrayList<>(),
                    PhotonPoseEstimator.PoseStrategy.AVERAGE_BEST_TARGETS), false);
    ...
    public void updateInputs(VisionInputs inputs) {
        if (calculateScoreParams && LimelightHelpers.getTV(name)) {
            inputs.poseFieldOriented = LimelightHelpers.getBotPose(name);
            inputs.distanceToSpeaker = inputs.poseFieldOriented
                    .getTranslation().toTranslation2d().getDistance(VisionConstants.getSpeakerPose());
            inputs.yawToSpeaker = Rotation2d.fromDegrees(LimelightHelpers.getTX(name));
            parameters = Optional.of(new ScoreParameters(inputs.distanceToSpeaker, inputs.yawToSpeaker));
        } else {
